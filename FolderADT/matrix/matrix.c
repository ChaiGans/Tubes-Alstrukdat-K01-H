...
boolean isSparse(Matrix m) {
   ...
   hanya maksimal 5% dari memori matriks yang efektif bukan bernilai 0 */
    ...
    if (count <= 0.05*ROW_EFF(m)*COL_EFF(m)) {
        return true;
    }
    return false;
}
...
Matrix cofMatrix(Matrix m, int a, int b) {
    int i, j, k, l;
    k = 0;
    l = 0;
    Matrix m3;
    createMatrix(ROW_EFF(m)-1, COL_EFF(m)-1, &m3);
    for (i=0; i<ROW_EFF(m); i++) {
        for (j=0; j<COL_EFF(m); j++) {
            if (i!=a && j!=b) {
                ELMT(m3, k, l) = ELMT(m, i, j);
                l += 1;
            } 
        }
        if (l == COL_EFF(m3)) {
...
float determinant(Matrix m) {
    ...
    if (ROW_EFF(m) == 2 && COL_EFF(m) == 2) {
        ...
        for (j=0; j<COL_EFF(m); j++) {
            if ((i+j)%2 == 0) {
                sum += ELMT(m, i, j)*determinant(cofMatrix(m, i, j));
            } else if ((i+j)%2 == 1) {
                sum += (-1)*ELMT(m, i, j)*determinant(cofMatrix(m, i, j));
            }  
        }
    }
}
